...
**読み取りパスは完全インメモリ化**: users / comments / posts / 集計 / 投稿カード HTML（一覧・詳細）すべてキャッシュ。
DB は INSERT（投稿・コメント・登録・BAN）のみ。セッションは署名付き Cookie + JSON。

エンドポイント別 CPU（pprof, 直近）: `POST /`(画像書き込み) 約15% / `GET /` 約12% / `GET /posts/:id` 約10% / `GET /@` 約9%。
フラットの最大は **Syscall（ネットワーク I/O）** で、もはや単一の大きな改善余地は無い。

### まだ伸ばせそうな箇所（残り・いずれも中〜小）

- **`POST /`（画像アップロード）の書き込みコスト**: 800KB の画像ファイル書き込みが主。非同期化や軽量化は要検討（ベンチの画像取得タイミングと整合を取る必要あり）。
- **投稿リスト HTML のページ単位キャッシュ**: 現状は投稿カード断片キャッシュ + 連結 + CSRF 置換（[17](./17-html-fragment-cache.md)）。トップ先頭ページなど高頻度ページを丸ごとキャッシュすれば連結コストも省ける（無効化設計が必要）。
- **gzip / レスポンス圧縮の本番A/B**: CPU バウンドかネットワークかで損益が変わるため、本番（別ホスト）で計測して判断。
- **アロケーション削減（GC）**: `renderPostList` のバッファを `sync.Pool` で再利用するなど。

> 計測は `bin/analysis-mode.sh`、公式スコア計測は `bin/scoring-mode.sh` を使う（[22](./22-scoring-mode-ab.md)）。
