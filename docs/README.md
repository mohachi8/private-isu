...
| フェーズ | 内容 |
|---|---|
| Phase 1（完了） | インデックス追加（→ スコア 16,447 / fail 0）|
| Phase 2 | 画像の静的配信化（nginx で `/image/*` を直接返す）|
| Phase 3 | N+1 解消（users のまとめ取得 / JOIN 化）|
| Phase 4 | クエリの LIMIT 付与・JOIN 最適化・`interpolateParams` 等 |
